...
export default class StreamingMonitor extends LightningElement {
    ...
    connectedCallback() {
        ...
        getAllEventChannels().then(allChannels => {
    ...
    handleStreamingError(error) {
        const errorMessage = (error.subscription ? error.subscription + ' - ' : '')
            + (error.error ? error.error : 'See browser\'s dev console for details');
        ...
        if (error.channel === '/meta/subscribe' && error.error && error.error.indexOf('400::The channel specified is not valid') !== -1) {
            ...
            if (!this.ignoreCdcSubscribeErrors && isCDCChannel(subChannel)) {
                this.notify('error', 'Streaming API error', `Failed to subscribe to ${subChannel}. Is the CDC event active?`);
            }
        } else { // Notify error with a toast
    ...
    handleSubscribeAll(event) {
        ...
        Promise.all(subscribePromises).then(subscriptions => {
            ...
            this.notify('success', 'Successfully subscribed to all channels', 'Hold on for a sec as we remove CDC channels that are not enabled.');
    ...
    handleStreamingEvent(streamingEvent) {
        ...
        console.log('Received streaming event: ', JSON.stringify(streamingEvent));
        // Save event
    ...
    handlePublish(event) {
        ...
        publishStreamingEvent(eventParams).then(() => {
            this.notify('success', `Successfully published event ${eventParams.eventName}`);
